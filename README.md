...
2. 끝입니다. 누를 버튼이 하나 더 있지 않습니다.

   영상을 넣으면 방송 규격에 맞는지 자동으로 확인하고, 필요한 만큼만 준비합니다.
   **원본 파일은 절대 수정되지 않습니다.** 준비가 3초 안에 끝나면 진행률도
   보여주지 않고 바로 `송출 준비 완료`가 됩니다.

   준비는 **필요한 것만** 합니다. 이미 방송 규격(H.264 / 지정 해상도 ·
   프레임레이트 / yuv420p / 48kHz 스테레오 AAC)인 영상 — YouTube에서 내려받은
   보통의 MP4가 대개 여기 해당합니다 — 은 화면도 소리도 다시 인코딩하지 않고
   컨테이너만 다시 씁니다. 화면만 또는 소리만 어긋난 영상은 어긋난 쪽만
   인코딩합니다.

   | 영상 상태 | 하는 일 | 60초 기준 (CPU, libx264) |
   | --- | --- | --- |
   | 이미 방송 규격 | 컨테이너만 다시 씀 | **0.50초** (이전 23.9초) |
   | 소리만 다름 | 소리만 인코딩 | 2.69초 (이전 18.3초) |
   | 화면·소리 모두 다름 | 둘 다 인코딩 | 27.2초 (변화 없음) |

   컨테이너를 다시 쓰는 일은 디스크 작업이라 실시간의 약 119배로 진행됩니다 —
   1시간짜리 영상이 30초 남짓, 5분짜리가 2.5초 남짓입니다.

   이 마지막 한 번의 복사는 건너뛸 수 없습니다. 규격에 맞는 원본이라도 소리
   트랙이 화면과 같은 프레임에서 끝나지 않기 때문에, 원본을 그대로 이어붙이면
   반복 지점마다 타임스탬프가 어긋납니다
   (`a_source_file_used_untouched_breaks_the_loop`가 이걸 고정합니다).

   준비가 오래 걸리는 영상에는 진행률과 남은 시간을 보여줍니다. 어떤 엔진
   (NVIDIA GPU · Intel Quick Sync · AMD GPU · CPU)을 쓸지는 실제로 동작하는
   인코더를 찾아 자동으로 정하며, **설정 → 고급**에서 확인만 할 수 있습니다.
...
| 증상 | 원인 | 해결 |
| --- | --- | --- |
| `LL-STREAM-003` | 아직 준비되지 않은 영상이 있음 | 플레이리스트에서 `방송 준비` 실행 |
...
준비된 영상은 1080p30 기준 시간당 약 2.8 GB를 사용합니다 (영상 6 Mbps +
...
같은 폴더에 데이터베이스(`louver.db`), 준비된 영상 캐시(`cache/`), 세션 상태
...
| `LL-MEDIA-005` | 방송 준비 실패 |
| `LL-MEDIA-006` | 방송 준비가 취소됨 |
| `LL-STREAM-001` | 방송 엔진을 시작하지 못함 |
| `LL-STREAM-002` | 방송이 예기치 않게 중단됨 |
| `LL-STREAM-003` | 아직 준비되지 않은 영상이 있음 |

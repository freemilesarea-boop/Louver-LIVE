...
`crates/louver-core/src/youtube/quota.rs`가 하루 사용량을 세고, 한도에 닿기 전에
스스로 멈춥니다. 중요한 설계 두 가지:
...
- **계량이 전송 계층에 있습니다.** `MeteredClient`가 `HttpClient`를 감싸고, 앱에서
  ...
  씁니다.
- **비용은 일부러 높게 잡았습니다.** Google의 메서드별 단가표를 이 저장소에서
  받을 수 없어(`developers.google.com` 접근 불가), 읽기 1 · 쓰기 50 · 검색 100으로
  두고 **모르는 호출은 쓰기 가격**으로 계산합니다. 낮게 잡으면 한도를 넘어 403을
  맞고, 높게 잡으면 일찍 멈춥니다. 안전한 쪽은 하나뿐입니다.
- 마지막 500 단위는 예비로 남겨, 제목을 적용하는 도중이 아니라 앱이 스스로
  판단해서 멈추게 합니다.

실측 기준 하루 사용량: 방송 1회의 메타데이터 적용(읽기 3 + 쓰기 2 ≈ 103 단위)과
20분 간격 채팅 72건(3,600 단위) = 약 **3,700 단위**. 한도의 40% 미만입니다.
(`a_days_realistic_use_fits_comfortably`, `a_days_worth_of_chat_fits_inside_the_free_allowance`)

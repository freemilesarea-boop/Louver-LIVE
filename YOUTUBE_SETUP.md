...
3. **API 및 서비스 → OAuth 동의 화면**
   ...
   > **테스트 모드의 7일 제한.** 앱을 "테스트" 상태로 두면 Google이 발급한
   > refresh token이 **7일 뒤 만료**됩니다. 그때는 앱에 `LL-YOUTUBE-002`가
   > 뜨고, 설정에서 다시 연결하면 됩니다. 24시간 방송을 계속 돌리실 거라면
   > OAuth 동의 화면을 **게시(프로덕션)** 상태로 바꾸세요. 범위가
   > `youtube.force-ssl` 하나뿐이고 본인 채널에만 쓰므로 Google 심사 없이
   > 게시할 수 있습니다.

...
### 같은 커밋, 같은 URL, 1초 차이 — 503 이 간헐적이었다는 증거

추측이 아니라 같은 run 시각표입니다. commit `24dc2c7` 하나에서:

| 시각 (UTC) | job | 결과 |
| --- | --- | --- |
| 08:53:10 – 08:53:17 | CI `sidecars (x86_64-pc-windows-msvc)` | 성공 |
| 08:54:02 – 08:54:10 | CI `verify (windows-latest)` 의 `fetch-ffmpeg.mjs --force` | 성공 |
| **08:54:11 – 08:54:12** | Release `Windows x64` 의 sidecar step | **503** |

세 job 모두 `windows-latest` 에서, 같은
`ffmpeg-release-essentials.zip` 을 받습니다. **08:54:10 에 성공한 다운로드가
08:54:11 에 503** 입니다. 호스트가 간헐적으로 503 을 내고 있었고, 한 번만
묻는 fetch 는 동전 던지기였습니다. CI 가 그 시점에 초록이었던 것도 같은
이유입니다 — 운이 좋았을 뿐입니다.

이것이 미러가 아니라 재시도가 맞는 수정인 이유이기도 합니다. 호스트는
내려가 있지 않았습니다. 요청 대부분에 정상 응답하고 있었고, 릴리스는
그중 하나의 실패에 걸렸을 뿐입니다.

## 영상 등록 UX — "최적화"를 사용자에게서 걷어냄 (2026-09-22, v1.0.6)

### 사용자가 하는 일

영상 선택 → 기다림 → 끝. `+ 영상 추가` 하나가 전부이고, 두 번째 버튼은
존재하지 않습니다. `add_media` 한 번의 호출이 import 와 준비를 모두 합니다.

### 발견한 버그 — 원본을 그대로 송출하면 루프가 깨집니다

§4 의 "이미 호환된 파일은 전혀 건드리지 않는다"를 그대로 구현하려다
기존 코드의 실제 결함을 찾았습니다. `MediaStatus::Compatible` 인 파일은
`normalized_path` 없이 **사용자 원본 경로가 그대로 concat manifest 에**
들어가고 있었습니다.

새 테스트 `a_source_file_used_untouched_breaks_the_loop` 가 이걸 잡습니다.
프로파일과 코덱·해상도·프레임레이트·픽셀 포맷·타임베이스가 **전부 일치하는**
원본을 준비된 항목과 번갈아 이어붙이면:

```
Non-monotonic DTS in output stream 0:1; previous: 3008, current: 3000
Non-monotonic DTS in output stream 0:1; previous: 6029, current: 6018
Non-monotonic DTS in output stream 0:1; previous: 9026, current: 9018
```

같은 파일을 **remux 한 뒤**에는 한 건도 나오지 않습니다. 차이는 정규화가
붙이는 `-t <whole-frame duration>` 과 `-avoid_negative_ts make_zero` 입니다.
원본의 AAC 트랙은 영상과 같은 프레임에서 끝나지 않고, 그 어긋남이 이어붙임
지점마다 쌓입니다. 어떤 원본도 이 두 가지를 스스로 갖고 있지 않습니다.

그래서 §4 의 "container/timescale 정리만 remux" 가 문자 그대로 맞습니다 —
remux 보다 싼 경로는 없고, 있어서도 안 됩니다. `is_broadcast_ready` 는 이제
`Normalized` 만 인정하며, 예전 버전이 남긴 `compatible` 행은 송출되는 대신
한 번 더 복사됩니다.

### 키프레임 간격 구멍도 같이 막았습니다

`check_compatibility` 는 키프레임 간격을 보지 않았고 `plan_transcode` 는
봤습니다. 그래서 키프레임이 2분 간격인 파일이 import 시점에 "준비 완료"로
표시될 수 있었습니다. 이제 `readiness_for` 가 두 질문에 하나의 답을 냅니다.

### 실측 (libx264, GPU 없음, 60초 소스)

| 소스 | mode | 걸린 시간 | 실시간 대비 | 이전 |
| --- | --- | --- | --- | --- |
| 이미 방송 규격 | `remux` | **0.50s** | 118.8× | 23.86s |
| 소리만 규격 밖 | `audio-only` | 2.69s | 22.3× | 18.26s |
| 화면·소리 모두 밖 | `full-transcode` | 27.22s | 2.2× | 15.89s |

118.8× 는 1시간 영상 약 30초, 5분 영상 약 2.5초입니다. §6 의 "호환 파일 몇 초
내"는 5분급 영상에서는 맞고 1시간급에서는 30초입니다 — 디스크 작업이라
디스크 속도가 결정합니다. 추정하지 않고 측정한 값입니다.

### 문구

| 이전 | 이후 |
| --- | --- |
| 영상 최적화 / 방송용으로 최적화 | 방송 준비 |
| 최적화 중 | 방송에 맞게 준비 중 |
| 최적화 필요 | 방송 준비 필요 |
| 이미 방송에 최적화된 영상입니다 | 바로 사용할 수 있는 영상입니다 |
| 최적화 인코더 | 사용 중인 변환 엔진 |

`최적화`, `인코딩`, `H.264`, `AAC`, `remux` 는 일반 사용자 화면에서
사라졌습니다. `format.test.ts` 의 `never calls preparation "최적화"` 가
상태 라벨에 대해 이걸 잠급니다. 엔지니어용 어휘는 로그의
`mode=remux|video-only|audio-only|full-transcode|cached` 에만 남습니다.

### 진행률

3초가 지나야 진행률 UI 가 나타납니다. 대부분의 추가는 그 전에 끝나므로
막대가 번쩍이지 않습니다. 표시될 때는 `영상 2/5`, 퍼센트, `약 1분 남음`.
속도 배수(`3.2x`)는 사용자 화면에서 뺐습니다 — 읽을 이유가 없는 숫자입니다.

### 저장 공간 모달 제거

준비가 자동이 된 이상 시작 전 확인 모달은 누를 버튼이 하나 더 있는 것과
같습니다. 공간 부족 시 거부하는 로직은 `optimize_media` 안에 그대로 있고,
필요/여유 용량을 담은 오류가 그대로 올라옵니다. 검사를 지운 것이 아니라
클릭을 지웠습니다.

...
## YouTube 방송 정보와 자동 채팅 (선택 기능)

여기부터는 없어도 방송이 되는 부분입니다. 스트림 키만으로 송출은 이미 끝났고,
아래는 제목·설명·태그를 앱에서 바꾸거나 채팅을 자동화하고 싶을 때만 봅니다.
...
실제 YouTube에 반영하려면 **설정 → YouTube 고급 기능 (선택)**에서 계정을
연결해야 합니다. 준비
